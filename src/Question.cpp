...
void Question::checkSolution(Mode mode)
{
    if (mode == Mode::ProvidedTestCases) doProvidedTestCases();
    else doCustomInput();

    cout << endl << "(ENTER to quit)" << endl;
    cin.get();
}
...
void Question::doProvidedTestCases()
{
    ...
    while (inputs_it != inputs.end())
    {
        ...
        // Time solution() execution duration
        auto start_time = chrono::high_resolution_clock::now();
        string actual_out = solution_func(splited);
        auto stop_time = chrono::high_resolution_clock::now();
        int duration = int(chrono::duration_cast<chrono::microseconds>(stop_time - start_time).count() / 1000.0);
...
void Question::doCustomInput()
{
    /* This only provides a wrapper for the solution function, as you need to do your own input handling and timing. */

    string output = solution_func(vector<string>());
